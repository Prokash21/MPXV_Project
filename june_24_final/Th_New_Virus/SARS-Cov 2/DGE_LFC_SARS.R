...
setwd("E:/DWCT/DEG/GSE219036_6_june/JUNE_24/Th_New_Virus/SARS-Cov 2")
...
# read in counts data
counts_data_m <- read.csv('Count_Cov_Final.csv' )
head(counts_data_m)
...
# read in sample info
colData <- read.csv('Meta_Cov_Final.csv', header = TRUE, row.names = 1)
...
#set the factor level
dds$treatment <- factor(dds$treatment, levels = c ("NONCOVID","COVID")) 
...
dds$treatment <- relevel(dds$treatment , ref = "NONCOVID")
dds$treatment
...
resLFC <- lfcShrink(dds, coef ="treatment_COVID_vs_NONCOVID"  , type = "apeglm")
...
write.csv(resLFC,"resLFC_SARS.csv")
