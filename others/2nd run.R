...
# Order the dataframe for top 20 up ragulated gene
ordered_up_resLFC <- resLFC[(order(resLFC$diffexpressed[resLFC$log2FoldChange>0.1 & resLFC$padj<0.05]), ][1:20, ]

# Select the first 20 genes
selected_genes <- ordered_resLFC[1:20, ]
selected_genes <-row.names(selected_genes)
selected_genes


# Assuming your dataframe is named 'df' and the column containing the word is named 'ColumnName'
up_hits <- subset(resLFC, grepl("UP", diffexpressed, ignore.case = TRUE))
...
top_up_hits <- resLFC[(order(resLFC$diffexpressed),][1:20,]
top_20_up <- resLFC
top_hits <- deseq_results[order(deseq_results$padj),][1:20,]
...
down_hits <- subset(resLFC, grepl("DOWN", diffexpressed, ignore.case = TRUE))


#up_hits <- resLFC[(resLFC$diffexpressed),][1:20,]
down_hits <-row.names(up_hits)
down_hits

ensembl_up <- resLFC[resLFC$diffexpressed == 'UP', 1]
ensembl_up <-row.names(ensembl_up)
ensembl_up
