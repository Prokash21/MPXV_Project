...
setwd("D:/DEG/GSE219036_6_june/cell line")

#load the count data

count_data <- read.csv("merged_cell_line.csv", header=TRUE,row.names = 1)
...
sample_info <- read.csv("final_cell_line_serial.csv",header = TRUE,row.names = 1)
colnames(sample_info)
head(sample_info)
#set actor levels

sample_info$Treatment <- factor(sample_info$Treatment)
sample_info$Cell_line <- factor(sample_info$Cell_line)
...
dds <- DESeqDataSetFromMatrix(countData = new_count_data, colData = sample_info, design = ~ Cell_line + Treatment)
...
hist(deseq_results_ordered$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
...
# create histogram plot of p-values
hist(filtered$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
     xlab = "", ylab = "", main = "GSE219036 Frequencies of padj-values")
...
plotPCA(vsd,intgroup=c("Cell_line", "Treatment"))
...
annot_info <- as.data.frame(colData(dds)[,c("Cell_line", "Treatment")])
pheatmap(assay(rld)[top_hits,],cluster_rows = FALSE,show_rownames = TRUE,cluster_cols = FALSE,
...
resLFC <- lfcShrink(dds, coef = "Cell_line_Normal.Human.Epidermal.Keratinocytes_vs_iPS.cell.derived.colon.organoids" , type = "apeglm")
