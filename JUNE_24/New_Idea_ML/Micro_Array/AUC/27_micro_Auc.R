...
getwd()
library("missForest")
setwd("E:/DWCT/DEG/GSE219036_6_june/JUNE_24/New_Idea_ML/Micro_Array/AUC")
#BiocManager::install("pROC")
#package_version("pROC")
library(pROC)

X <- read.csv("27_micro_P_1_run.csv")
###########################################################################
...
# Check the structure to confirm all columns are now numeric
str(X)
...
gene_data_clean <- imputed_data$ximp

# Reattach the ID column if needed
X <- cbind(ID, gene_data_clean)

# Inspect the cleaned data
head(gene_data_clean)
